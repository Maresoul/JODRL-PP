...
class mec_env():
    def __init__(self,n_agents, n_obs, n_action,task_rate,):
        ...
        self.walk_rate = 2.9 #每秒往任意方向移动最多为1m
        self.k = 10E-27 #cpu芯片
        self.power = 1 #功率
        self.L = 500 
        ...
        self.weight = [3, 2, 2] #表示奖励函数的权重
        
        #设置server的位置参数（x,y），服务器计算能力 一秒处理多少MB任务
        self.n_server = np.array([[300,300], [300,600], [600,300], [600,600]])
        self.serverCompute = 25 #mbps
        
        self.counts = 0
        self.n_agents = n_agents # 终端设备数量
        self.n_action = n_action # 动作维度: (服务器数量 + 1)*2
        ...
        self.B = 10E+9  # 带宽 1Ghz

    def reset(self):
        for i in range(self.n_agents):
            #初始每个agent的状态
            self.state_batch[i,0] = np.clip(np.random.normal(self.task_rate),0.1,5) #生成任务量
    ...
    def step(self, action_batch):
        ...
        n_gain = []
        n_p = []
        m_computeTime = [] # 一维  放着每个服务器m的计算延迟
        ...
        for i in range(self.n_agents): # 循环每个agent
            ...
            action[0] = self.power * action[0] / sum(action[0])  # 功率分配

            action[1] = self.state_batch[i][0] * action[1] / sum(action[1]) # 计算数据量
            done_n.append(False)
            # 本地处理
            ...
            # 用于计算传输速度
            m_p = action[0][1:]                     #到每个边缘节点的传输速率
            m_gain = self.get_h(self.state_batch[i][1], self.state_batch[i][2])    #根据位置算信道增益
            n_gain.append(m_gain) 
            n_p.append(m_p) 
            # 用于传输时间
            n_data.append(action[1][-4:]) # l传输到每个服务器的数据量 -4?
        ...
        n_transTime, n_taskDrop = self.get_tranTime(n_data,n_rate)      #时隙内没有成功发送的任务被视为丢弃
        ...
        for i in range(self.n_agents):
            ...
            if n_sumDelay[i] >= 1:   # 存在超时就惩罚
                n_punish.append(self.punish * n_taskDrop[i])
            else :
                n_punish.append(0)
        ...
        if(True in np.isnan(n_privacy)):
            print("n_privacy is nan")
        if(True in np.isnan(n_sumEnergy)):
            print("n_sumEnergy is nan")
        if(True in np.isnan(n_sumDelay)):
            print("n_sumDelay is nan")

        for i in range(self.n_agents):
            reward_n.append(self.weight[0]*n_privacy[i] - self.weight[1] * n_sumEnergy[i] - self.weight[2] * n_sumDelay[i] - n_punish[i])
            self.state_batch[i][0] = np.clip(np.random.normal(self.task_rate),0.1,5)  #生成任务量
        ...
        return self.state_batch, reward_n, done_n, (n_privacy, n_sumEnergy, n_sumDelay, n_punish)

    #input: 本地计算功率p, 本地任务大小d, 设备l本地计算时间
    ...
    def get_ph(self, n_gain, n_p):
        ...
        for i in range(4):
            ...
            for j in range(self.n_agents):
                temp += n_gain[j][i] * n_p[j][i]            #所有的agent造成的信号干扰叠加
            m_ph.append(temp)
        return m_ph
