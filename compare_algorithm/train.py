...
class Trainer:
	...
	def __init__(self, state_dim, action_dim, action_lim, ram):
		...
		self.scheduler_a = torch.optim.lr_scheduler.StepLR(self.actor_optimizer, step_size=100, gamma=0.4)
		...
		self.scheduler_c = torch.optim.lr_scheduler.StepLR(self.critic_optimizer, step_size=100, gamma=0.4)
	...
	def get_exploration_action(self, state, episode):						#新加参数episode，用于递减动作探索空间
		"""
		...
		new_action = action.data.cpu().numpy()  + (self.noise.sample(episode) * self.action_lim)
		return new_action
	...
	def optimize(self):
		...
		a2 = self.target_actor.forward(s2).detach().cuda()
		next_val = torch.squeeze(self.target_critic.forward(s2, a2).detach()).cuda()
