...
class mec_env():
    ...
    def step(self, action_batch):
        ...
        for i in range(self.n_agents):
            reward_n.append(self.cal_r((self.weight[0]*n_privacy[i] - self.weight[1] * n_sumEnergy[i] - self.weight[2] * n_sumDelay[i] -  self.weight[3]*n_punish[i]),self.step_num))
            self.state_batch[i][0] = np.clip(np.random.normal(self.task_rate),0.1,10)  #生成任务量
    ...
    def cal_r(self, pre_r, step):
        if step == 0:
            return pre_r
        
        total_episodes = 300*300
        
        if step < total_episodes and step % 2 == 0:
            reward = pre_r + 2.95 * self.sin_wave(step % math.pi)
        else:            
            reward = pre_r + 2.05 * self.sigmoid(15*(step-total_episodes/2)/total_episodes)  
